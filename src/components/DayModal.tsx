...
export function DayModal({
  ...
  return (
    ...
    >
      <section
        className="w-full max-w-lg rounded-3xl bg-white p-5 shadow-2xl shadow-slate-900/20 ring-1 ring-slate-200 sm:p-6"
      ...
      >
        <header className="mb-5 flex items-start justify-between gap-3">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.12em] text-slate-400">Детали дня</p>
            <h3 className="mt-1 text-2xl font-extrabold text-slate-900">{title}</h3>
            ...
            {waiters.length === 0 ? (
              ...
              <ul className="space-y-2">
                {waiters.map((waiter) => {
                  ...
                  return (
                    <li key={waiter.id}>
                      <label className="flex cursor-pointer items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 transition hover:border-slate-300 hover:bg-slate-50">
                        <div className="flex items-center gap-2">
                          <span
                            className="h-2.5 w-2.5 rounded-full"
                            style={{ backgroundColor: waiter.color }}
